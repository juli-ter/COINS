...
def count_changepoints_by_noise(curr_var_arr: np.ndarray) -> dict:
    valid = curr_var_arr[~np.isnan(curr_var_arr)]
    unique_vars = np.unique(valid)

    counts = {}
    labels = ["low", "medium", "high"]

    for label, var in zip(labels, unique_vars):
        counts[label] = int(np.sum(valid == var))

    counts["excluded_nan"] = int(np.sum(np.isnan(curr_var_arr)))
    counts["total_valid"] = int(len(valid))
