...
def coins_plot_subject_adjustments(adjusts: np.ndarray, jump_sizes: np.ndarray, options: Options) -> tuple[plt.Figure, plt.Figure]:
    ...
    for j in jump_sizes:
        ...
        ax2.text(4.05, j, f'{j:.2f}', va='center')
...
def coins_plot_block_changepoint_counts(block_counts_df: pd.DataFrame) -> plt.Figure:
    fig, ax = plt.subplots(figsize=(13, 6))

    ordered_df = block_counts_df.sort_values(["session", "block"]).reset_index(drop=True)

    labels = [
        f"S{int(row.session)} B{int(row.block)}\n{row.volatility}"
        for row in ordered_df.itertuples()
    ]

    x = np.arange(len(ordered_df))

    low = ordered_df["low"].to_numpy(dtype=float)
    medium = ordered_df["medium"].to_numpy(dtype=float)
    high = ordered_df["high"].to_numpy(dtype=float)
    excluded = ordered_df["excluded_nan"].to_numpy(dtype=float)

    ax.bar(x, low, label="low noise")
    ax.bar(x, medium, bottom=low, label="medium noise")
    ax.bar(x, high, bottom=low + medium, label="high noise")
    ax.bar(
        x,
        excluded,
        bottom=low + medium + high,
        label="excluded: noise changed after CP",
        alpha=0.45,
    )

    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=45, ha="right")
    ax.set_xlabel("Session and block")
    ax.set_ylabel("Number of changepoints")
    ax.set_title("Changepoints by local noise level in each block")
    ax.legend()

    return fig


def coins_plot_group_changepoint_counts(total_counts_df: pd.DataFrame) -> plt.Figure:
    ...
    values = total_counts_df["n_changepoints"].to_numpy(dtype=float)
    ...
    ax.set_ylabel("Number of changepoints")
    ax.set_title("Changepoints by local noise level across all participants")
...
def coins_plot_group_adjustments(avg_adjusts: np.ndarray, se_adjusts: np.ndarray, jump_sizes: np.ndarray, options: Options) -> tuple[plt.Figure, plt.Figure]:
    ...
    for data, errors, color, linestyle, label in noise_data:
        for i_jump in range(data.shape[0]):
            current_label = label if i_jump == 0 else None
    ...
    for jump in jump_sizes:
        ax.axhline(jump, color='gray', alpha=0.3)
        ax.text(
            3.92,
            jump + 0.01,
            f'{jump:.2f}',
    ...
    ax.set_xlim(-1, 4)
    ax.set_xlabel('time from change point (s)')
    ax.set_ylabel('shield adjustment (rad)')
    ax.set_title('Effect of noise level across participants')
    ax.legend()
